# Task 6 - Add "MoveTargetsBack" function to move the targets back whilst maintaining list size (and takes away 2 points) - 4 marks
...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        ...
        UserInput = input("Enter an expression or MOVE: ")
        print()
        if UserInput == "MOVE":
            Targets, Score = MoveTargetsBack(Targets, Score)
            continue
...
def MoveTargetsBack(Targets, Score):
    newList = [-1]
    for val in Targets:
        newList.append(val)

    newList.pop()
    return newList, Score-2
