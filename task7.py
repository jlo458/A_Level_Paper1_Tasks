# Task 7 - When target successful (e.g. 15), user should be given choice to add that number/part of that number to their numbers allowed (15, 1, 5) - 7 marks
    ...
    while not GameOver:
        ...
        if CheckIfUserInputValid(UserInput):
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                IsTarget, Score, Result = CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score)
                if IsTarget:
                    ...
                    extraNums = list(str(Result))
                    extraNums.append(str(Result))
                    print(f"Possible extra numbers are: {extraNums}")
                    extraChoice = input("Enter which number you would like to add to available numbers (ENTER if none): ")
                    for val in extraNums:
                        if val in extraNums:
                            extraVal = val

                    NumbersAllowed = FillNumbers(NumbersAllowed, TrainingGame, MaxNumber, extraVal)
...
def FillNumbers(NumbersAllowed, TrainingGame, MaxNumber, extraVal):
    if TrainingGame:
        return [2, 3, 2, 8, 512]
    ...
    # Added Code
        ...
        NumbersAllowed.append(extraVal)
        return NumbersAllowed
