# Task 8 - Add challenge mode (1/4 chance challege mode happens, all numbers must be used. If completed, bonus 10 points, if not, score = score - 5) - 8 marks 
...
def PlayGame(Targets, NumbersAllowed, TrainingGame, MaxTarget, MaxNumber):
    ...
    while not GameOver:
        # Added Code
        chalMode = False
        challModeChoice = random.randint(1,4)
        if challModeChoice == 1:
            chalMode = True
            print("Challenge Mode Active!")
        ...
        if CheckIfUserInputValid(UserInput):
            ...
            if CheckNumbersUsedAreAllInNumbersAllowed(NumbersAllowed, UserInputInRPN, MaxNumber):
                IsTarget, Score = CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score, chalMode)
                if IsTarget:
...
def CheckIfUserInputEvaluationIsATarget(Targets, UserInputInRPN, Score, chalMode):
    # Added Code
    count = 0
    bonus = False
    if chalMode:
    ...
    if UserInputEvaluation != -1:
        for Count in range(0, len(Targets)):
            if Targets[Count] == UserInputEvaluation:
                if count != 5 and chalMode:
                    Score = -4
                    return False, Score

                if not bonus and count == 5:
                    Score += 10
                    bonus = True
